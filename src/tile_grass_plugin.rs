...
pub struct TileGrassPlugin;

impl Plugin for TileGrassPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<SpawnGrassTileEvent>()
            .add_event::<DespawnGrassTileEvent>()
...
pub struct SpawnGrassTileEvent(pub u32, pub u32);
pub struct DespawnGrassTileEvent(pub u32, pub u32);
...
fn spawn_tile(
    ...
    mut q_storage: Query<(Entity, &mut TileStorage), With<LayerGrass>>,
    mut spawn_event_reader: EventReader<SpawnGrassTileEvent>,
) {
...
fn despawn_tile(
    mut commands: Commands,
    mut q_storage: Query<&mut TileStorage, With<LayerGrass>>,
    mut spawn_event_reader: EventReader<DespawnGrassTileEvent>,
) {
    if spawn_event_reader.is_empty() {
        return;
    }
