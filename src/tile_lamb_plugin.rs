...
pub struct TileLambPlugin;

impl Plugin for TileLambPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<SpawnLambTileEvent>()
            .add_event::<DespawnLambTileEvent>()
...
pub struct SpawnLambTileEvent(pub u32, pub u32);
pub struct DespawnLambTileEvent(pub u32, pub u32);
...
fn spawn_tile(
    ...
    mut q_storage: Query<(Entity, &mut TileStorage), With<LayerLamb>>,
    mut spawn_event_reader: EventReader<SpawnLambTileEvent>,
) {
...
fn despawn_tile(
    mut commands: Commands,
    mut q_storage: Query<&mut TileStorage, With<LayerLamb>>,
    mut spawn_event_reader: EventReader<DespawnLambTileEvent>,
) {
    if spawn_event_reader.is_empty() {
        return;
    }
