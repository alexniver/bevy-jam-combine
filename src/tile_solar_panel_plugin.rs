...
pub struct TileSolarPanelPlugin;

impl Plugin for TileSolarPanelPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<SpawnSolarPanelTileEvent>()
            .add_event::<DespawnSolarPanelTileEvent>()
...
pub struct SpawnSolarPanelTileEvent(pub u32, pub u32);
pub struct DespawnSolarPanelTileEvent(pub u32, pub u32);
...
fn spawn_tile(
    ...
    mut q_storage: Query<(Entity, &mut TileStorage), With<LayerSolarPanel>>,
    mut spawn_event_reader: EventReader<SpawnSolarPanelTileEvent>,
) {
...
fn despawn_tile(
    mut commands: Commands,
    mut q_storage: Query<&mut TileStorage, With<LayerSolarPanel>>,
    mut spawn_event_reader: EventReader<DespawnSolarPanelTileEvent>,
) {
    if spawn_event_reader.is_empty() {
        return;
    }
