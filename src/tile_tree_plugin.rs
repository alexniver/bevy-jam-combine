...
impl Plugin for TileTreePlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<SpawnTreeTileEvent>()
            .add_event::<DespawnTreeTileEvent>()
...
pub struct SpawnTreeTileEvent(pub u32, pub u32);
pub struct DespawnTreeTileEvent(pub u32, pub u32);
...
fn spawn_tile(
    ...
    mut spawn_event_reader: EventReader<SpawnTreeTileEvent>,
) {
...
fn despawn_tile(
    mut commands: Commands,
    mut q_storage: Query<&mut TileStorage, With<LayerTree>>,
    mut spawn_event_reader: EventReader<DespawnTreeTileEvent>,
) {
    if spawn_event_reader.is_empty() {
        return;
    }
