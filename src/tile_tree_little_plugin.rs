...
pub struct TileTreeLittlePlugin;

impl Plugin for TileTreeLittlePlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<SpawnTreeLittleTileEvent>()
            .add_event::<DespawnTreeLittleTileEvent>()
...
pub struct SpawnTreeLittleTileEvent(pub u32, pub u32);
pub struct DespawnTreeLittleTileEvent(pub u32, pub u32);
...
fn setup_tilemap(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    commands
        ...
        .insert(LayerTreeLittle);
}

fn spawn_tile(
    mut commands: Commands,
    mut q_storage: Query<(Entity, &mut TileStorage), With<LayerTreeLittle>>,
    mut spawn_event_reader: EventReader<SpawnTreeLittleTileEvent>,
) {
    ...
    for spawn_event in spawn_event_reader.iter() {
        ...
        let tile_entity = commands
            ...
            .insert(TileTreeLittle(TREE_LITTLE_GROW_DAYS, TREE_LITTLE_HP))
            .id();
...
fn despawn_tile(
    mut commands: Commands,
    mut q_storage: Query<&mut TileStorage, With<LayerTreeLittle>>,
    mut spawn_event_reader: EventReader<DespawnTreeLittleTileEvent>,
) {
    if spawn_event_reader.is_empty() {
        return;
    }
