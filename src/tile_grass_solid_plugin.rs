...
pub struct TileGrassSolidPlugin;

impl Plugin for TileGrassSolidPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<SpawnGrassSolidTileEvent>()
            .add_event::<DespawnGrassSolidTileEvent>()
...
pub struct SpawnGrassSolidTileEvent(pub u32, pub u32);
pub struct DespawnGrassSolidTileEvent(pub u32, pub u32);
...
fn setup_tilemap(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    commands
        ...
        .insert(LayerGrassSolid);
}

fn spawn_tile(
    mut commands: Commands,
    mut q_storage: Query<(Entity, &mut TileStorage), With<LayerGrassSolid>>,
    mut spawn_event_reader: EventReader<SpawnGrassSolidTileEvent>,
) {
    ...
    for spawn_event in spawn_event_reader.iter() {
        ...
        let tile_entity = commands
            ...
            .insert(TileGrassSolid(GRASS_SOLID_DAYS))
            .id();
...
fn despawn_tile(
    mut commands: Commands,
    mut q_storage: Query<&mut TileStorage, With<LayerGrassSolid>>,
    mut spawn_event_reader: EventReader<DespawnGrassSolidTileEvent>,
) {
    if spawn_event_reader.is_empty() {
        return;
    }
