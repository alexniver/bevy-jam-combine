...
pub struct TileWormPlugin;

impl Plugin for TileWormPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<SpawnWormTileEvent>()
            .add_event::<DespawnWormTileEvent>()
...
pub struct SpawnWormTileEvent(pub u32, pub u32);
pub struct DespawnWormTileEvent(pub u32, pub u32);
...
fn spawn_tile(
    ...
    mut q_storage: Query<(Entity, &mut TileStorage), With<LayerWorm>>,
    mut spawn_event_reader: EventReader<SpawnWormTileEvent>,
) {
...
fn despawn_tile(
    mut commands: Commands,
    mut q_storage: Query<&mut TileStorage, With<LayerWorm>>,
    mut spawn_event_reader: EventReader<DespawnWormTileEvent>,
) {
    if spawn_event_reader.is_empty() {
        return;
    }
