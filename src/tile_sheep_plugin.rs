...
pub struct TileSheepPlugin;

impl Plugin for TileSheepPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<SpawnSheepTileEvent>()
            .add_event::<DespawnSheepTileEvent>()
...
pub struct SpawnSheepTileEvent(pub u32, pub u32);
pub struct DespawnSheepTileEvent(pub u32, pub u32);
...
fn spawn_tile(
    ...
    mut q_storage: Query<(Entity, &mut TileStorage), With<LayerSheep>>,
    mut spawn_event_reader: EventReader<SpawnSheepTileEvent>,
) {
...
fn despawn_tile(
    mut commands: Commands,
    mut q_storage: Query<&mut TileStorage, With<LayerSheep>>,
    mut spawn_event_reader: EventReader<DespawnSheepTileEvent>,
) {
    if spawn_event_reader.is_empty() {
        return;
    }
