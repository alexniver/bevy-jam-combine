...
pub struct TileSandPlugin;

impl Plugin for TileSandPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<SpawnSandTileEvent>()
            .add_event::<DespawnSandTileEvent>()
...
pub struct SpawnSandTileEvent(pub u32, pub u32);
pub struct DespawnSandTileEvent(pub u32, pub u32);
...
fn spawn_tile(
    ...
    mut q_storage: Query<(Entity, &mut TileStorage), With<LayerSand>>,
    mut spawn_event_reader: EventReader<SpawnSandTileEvent>,
) {
...
fn despawn_tile(
    mut commands: Commands,
    mut q_storage: Query<&mut TileStorage, With<LayerSand>>,
    mut spawn_event_reader: EventReader<DespawnSandTileEvent>,
) {
    if spawn_event_reader.is_empty() {
        return;
    }
